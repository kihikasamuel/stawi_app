defmodule StawiAppWeb.UserAuthTest do
  use StawiAppWeb.ConnCase, async: true

  alias StawiApp.Accounts
  alias StawiApp.Accounts.Scope
  alias StawiAppWeb.UserAuth

  import StawiApp.AccountsFixtures

  @remember_me_cookie "_stawi_app_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, StawiAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/app/home"
